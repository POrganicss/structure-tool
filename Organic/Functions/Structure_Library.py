...
Once = Smile.getsmiless('C1C=CC=CC=1C(=O)NC(C=C2)=CC=C2O', [
    14,9, 5], [x, x, OX], [0, 1, 1])
print(Once)
Twice = []
Fr = OX*3600
for result, f in zip(Once, Fr):
    smile_units = Smile.smile_divide(result)
    frg1, frg2 = Smile.keletsonsplit(smile_units, 4)
    maxnum = Smile.getmaxcon(result)
    Fragment = Smile.modifyfrag(f, maxnum)
    Twice.append("".join(frg1 + '('+Fragment+')' + frg2))
...
Three_times = []
Fr = OX*3600
for result, f in zip(Twice, Fr):
    smile_units = Smile.smile_divide(result)
    frg1, frg2 = Smile.keletsonsplit(smile_units, 3)
    maxnum = Smile.getmaxcon(result)
    Fragment = Smile.modifyfrag(f, maxnum)
    Three_times.append("".join(frg1 + '('+Fragment+')' + frg2))

xyz=Smile.toxyzs(Three_times)
