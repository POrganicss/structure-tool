...
class Gaussian:
    ...
    def run(gjf,proname,name,index=0):
        ...
        elif index==1:#监控计算的结果，但成功时。输出True和能量和坐标，当计算失败时，输出False和坐标
            
            if Gaussian.getsuccess(proname,name):
                return True,Gaussian.getxyz(proname,name),Gaussian.getenergy(proname,name)
            else:
    ...
    def getxyz(proname,name):
        #----------调用服务器端的Log_SSH.py运行----------
        rcode=[]
        chkpath = Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.chk'
        rcode.append('formchk '+chkpath+'\n')
        
        rcode.append('python3 /root/Log_xyz.py ')
        rcode.append(Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.log'+'\n')
        Datatransmission.Commandtransmission(''.join(rcode))
        
        #----------将服务器运算完的数据导入到本地----------
        path1 = Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.xyz'
        path2=path+'/Temp/'+name+'.xyz'
    ...
    def getenergy(proname,name):
        ...
        rcode=[]
        rcode.append('python3 /root/Log_engergy.py ')
        rcode.append(Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.log')
        Datatransmission.Commandtransmission(''.join(rcode))
        
        #----------将服务器运算完的数据导入到本地----------
        path1 = Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.eng'
    ...
    def getsuccess(proname,name):
        ...
        rcode=[]
        rcode.append('python3 /root/Log_success.py ')
        rcode.append(Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.log'+'\n')
        Datatransmission.Commandtransmission(''.join(rcode))
        
        #----------将服务器运算完的数据导入到本地----------
        path1 = Gaussian.LPath+'/'+proname+'/'+name+'/'+name+'.res'
        ...
        #----------将服务器运算完的数据导入到本地----------
        scode=[]
        scode.append('rm -f '+Gaussian.LPath+proname+'/'+name+'/'+name+'.res')
        Datatransmission.Commandtransmission(''.join(rcode))
    ...
    def multrunsmiles(smiles,proname,name='proj'):
        ...
        if jobnumber<128:
            pool = ThreadPool(jobnumber)
            paramenters={'cpu':'0-127','mem':'8GB','charge':'0','spin':'1',
