...
class Datatransmission:
    ...
    def Commandtransmission(content):
        ...
        ssh.connect(Datatransmission.hostname, Datatransmission.port, Datatransmission.username
                    , Datatransmission.password,look_for_keys=True)
    ...
    def exist(path):
        ...
        # 连接到远程服务器
        ssh.connect(Datatransmission.hostname, Datatransmission.port, 
                    Datatransmission.username, Datatransmission.password)
...
class LocalCommand:
    ...
    def execute_command(command):
        # 使用Popen创建进程，并与进程进行复杂的交互
        proc = subprocess.Popen(command,        # cmd特定的查询空间的命令
            stdin=None,                         # 标准输入 键盘
            stdout=subprocess.PIPE,             # -1 标准输出（演示器、终端) 保存到管道中以便进行操作
            stderr=subprocess.PIPE,             # 标准错误，保存到管道
            shell=True)
